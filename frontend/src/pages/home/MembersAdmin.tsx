...
export default function MembersAdmin({ isAdmin = true }: { isAdmin?: boolean }) {
  ...
  function startEdit(u: any) { setEditing(u); setEdit({ id:u.id, name:u.name, gender:u.gender, college:u.college, join_year:u.join_year, phone:'', birthday:'' }) }
  async function saveEdit() { if (!editing) return; await api.updateMe(edit.id, edit); setEditing(null); await load() }
  async function delUser(id: number) { if (!confirm('确认删除该团员？')) return; await api.deleteUser(id); await load() }
  ...
  return (
    <div>
      ...
      {isAdmin && editing && (
        <div className="panel card-form" style={{ position:'fixed', inset:'0', background:'rgba(0,0,0,0.25)' }} onClick={()=>setEditing(null)}>
          <div className="panel" style={{ maxWidth:'640px', margin:'80px auto' }} onClick={e=>e.stopPropagation()}>
