...
async function main() {
  ...
  try {
    const [rows] = await pool.query('SELECT u.id,u.role,u.name,u.student_no,u.gender,CONVERT(c.name USING utf8mb4) AS college,u.join_year FROM users u JOIN colleges c ON c.id=u.college_id ORDER BY u.id')
